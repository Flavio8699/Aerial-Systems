...
//  Aerial Systems
//
//  Created by Flavio Matias on 03/03/2022.
//
...
struct testView: UIViewRepresentable {
    ...
    var fpvView: UIView = UIView.init()
...
class FPVViewController: NSObject, ObservableObject, DJIVideoFeedListener, DJICameraDelegate, DJIVideoPreviewerFrameControlDelegate {
    
    var isRecording : Bool!
    
    let enableBridgeMode = true
    
    let bridgeAppIP = "192.168.178.81"
    
    @Published var testV = testView()
    ...
    func setupVideoPreviewer() {
        DJIVideoPreviewer.instance().setView(self.testV.fpvView)
    ...
    func productConnected(_ product: DJIBaseProduct?) {
        ...
        self.productName = newProduct.model ?? "nothing?"
        ...
        DJISDKManager.keyManager()?.getValueFor(batteryLevelKey!, withCompletion: { [unowned self] (value: DJIKeyedValue?, error: Error?) in
            ...
            self.battery = "\(value!.unsignedIntegerValue) %"
        })
